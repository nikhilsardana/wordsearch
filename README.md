...
# Errors
If the output isn't what you expect, first check `formatted.txt` to make sure your input has been entered/OCR'd correctly. If your `formatted.txt` is correct but the algorithm isn't finding a word, verify the word falls between `--min` and `--max` and you are using the correct flags.
