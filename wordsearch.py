...
def diagwords(grid, minw, maxw, numr, numc):
  ...
  for k in range(minw, maxw+1):
    for i in range(numcols):
      for j in range(numrows):
        out, c = diag(grid, i, j, k, numr, numc)
...
gmap = np.zeros((len(grid[0]), len(grid)))
